...
class Renderer {
  ...
  _motorway(){
    const ctx=this.ctx,net=this.network;
    const LANES=3,cW=LANES*LANE_W,fullW=cW*2+8;
    ...
    const wayPts=(way,nodePtFn)=>way; // pts already resolved in chainWays

    net.mainRoads.forEach(road=>{
      const chainWays=road.chainWays; // [[way0,way1,...],[way0,...]] each way has .polyline set? No — we need to reproject
      // chainWays stores original way objects; we need pts from chain0/chain1
      const c0=road.chain0||[]; const c1=road.chain1||[];

      // Draw verge first (full width behind both chains)
      drawPoly(c0,cW+28,'#c8d8b8');
      if(c1.length) drawPoly(c1,cW+28,'#c8d8b8');

      // Road surface
      drawPoly(c0,cW,'#7a8599');
      if(c1.length) drawPoly(c1,cW,'#7a8599');

      // Central reservation line between the two chains
      drawPoly(c0,4,'#a8c090');

      // Edge lines and lane dashes
      [c0,c1].filter(c=>c.length).forEach(pts=>{
        drawPoly(pts,1.5,'rgba(255,255,255,0.85)');
        drawPoly(pts,1,'rgba(255,255,255,0.3)',[14,10]);
      });
      ...
      [c0,c1].filter(c=>c.length>=2).forEach(pts=>{
        ...
        for(let i=1;i<pts.length;i++){
          ...
          d+=Math.hypot(dx,dy);
          if(d>100){d=0;ctx.save();ctx.translate((pts[i].x+pts[i-1].x)/2,(pts[i].y+pts[i-1].y)/2);ctx.rotate(Math.atan2(dy,dx));ctx.beginPath();ctx.moveTo(7,0);ctx.lineTo(-3,-3);ctx.lineTo(-3,3);ctx.closePath();ctx.fill();ctx.restore();}
        }
      });
    });

    // Slip roads — draw stitched polylines directly
...
class OSMFetcher {
  ...
  static async geocode(query) {
    ...
    const r=await fetch(url,{headers:{'User-Agent':'IntersectSim/1.0'}});
  ...
  static async fetchRoads(lat,lon,radius=500) {
    for(const b of OSMFetcher._BUNDLED){
      const dist=Math.hypot((lat-b.lat)*111320,(lon-b.lon)*111320*Math.cos(lat*Math.PI/180));
      if(dist<1000){console.log(`Using bundled data for ${b.name} (${dist.toFixed(0)}m away)`);return b.data;}
    }
  ...
  static buildNetwork(osmData,lat,lon,W,H) {
    ...
    // Step 1: project with initial scale to find data bounds
    const initScale=Math.min(W,H)/2/500;
    const cosLat=Math.cos(lat*Math.PI/180);
    const initProj=(nLat,nLon)=>({
      x:W/2+(nLon-lon)*111320*cosLat*initScale,
      y:H/2-(nLat-lat)*111320*initScale,
    });

    // Collect all road node positions
    const allPts=[];
    allRoadWays.forEach(w=>w.nodes.forEach(nid=>{
      const n=nodeMap[nid];
      if(n) allPts.push(initProj(n.lat,n.lon));
    }));
    ...
    if(allPts.length>1){
      ...
      const fitScale=Math.min((W-2*pad)/dataW,(H-2*pad)/dataH);
      scale=initScale*fitScale;
      // Data centre in initial projection coords
      const dataCX=(minX+maxX)/2, dataCY=(minY+maxY)/2;
      // Convert data centre back to lat/lon, then reproject with new scale
      // Offset: canvas centre should map to data centre
      cx=W/2-(dataCX-W/2)*fitScale;
      cy=H/2-(dataCY-H/2)*fitScale;
    }

    const project=(nLat,nLon)=>({
      x:cx+(nLon-lon)*111320*cosLat*scale,
      y:cy-(nLat-lat)*111320*scale,
    });
