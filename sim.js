...
class Renderer {
  ...
  _motorway(){
    ...
    // Fixed visual widths — look good regardless of zoom scale
    const MW=18;   // motorway carriageway width px
    const SL=8;    // slip road width px

    const drawPoly=(pts,lw,style,dash=[])=>{
    ...
    // Draw slip road verges first (behind motorway)
    net.slipRoads.forEach(slip=>{
      const pts=slip.renderPts||[{x:slip.bx,y:slip.by},{x:slip.tx,y:slip.ty}];
      if(pts.length<2)return;
      drawPoly(pts,SL+8,'#c8d8b8');
    });

    net.mainRoads.forEach(road=>{
      const c0=road.chain0||[]; const c1=road.chain1||[];
      // Verge (widest, drawn first)
      drawPoly(c0,MW+14,'#c8d8b8');
      if(c1.length) drawPoly(c1,MW+14,'#c8d8b8');
      // Road surface
      drawPoly(c0,MW,'#6b7280');
      if(c1.length) drawPoly(c1,MW,'#6b7280');
      // Lane markings
      [c0,c1].filter(c=>c.length).forEach(pts=>{
        drawPoly(pts,1.2,'rgba(255,255,255,0.8)');
        drawPoly(pts,0.8,'rgba(255,255,255,0.35)',[10,8]);
      });
      // Central reservation
      if(c1.length){
        // Draw green stripe along each chain inner edge
        drawPoly(c0,3,'#8fad78');
        drawPoly(c1,3,'#8fad78');
      }
      // Direction arrows every ~80px along each chain
      [c0,c1].filter(c=>c.length>=2).forEach(pts=>{
    ...
    // Draw slip roads on top
    net.slipRoads.forEach(slip=>{
      const pts=slip.renderPts||[{x:slip.bx,y:slip.by},{x:slip.tx,y:slip.ty}];
      if(pts.length<2)return;
      drawPoly(pts,SL,'#8a919e');
      drawPoly(pts,0.8,'rgba(255,255,255,0.6)');
      // Direction arrow
      const mi=Math.floor((pts.length-1)*0.55);
      ...
      ctx.fillStyle=slip.type==='on-ramp'?'#16a34a':'#d97706';
...
class OSMFetcher {
  ...
  static async geocode(query){
    ...
    const d=await r.json();if(!d.length)throw new Error(`Location not found: "${query}"`);
    return{lat:parseFloat(d[0].lat),lon:parseFloat(d[0].lon),name:d[0].display_name};
  }
  static async fetchRoads(lat,lon,radius=500){
    for(const b of OSMFetcher._BUNDLED){const dist=Math.hypot((lat-b.lat)*111320,(lon-b.lon)*111320*Math.cos(lat*Math.PI/180));if(dist<1000){console.log(`Using bundled data for ${b.name}`);return b.data;}}
    ...
    for(const server of servers){
      try{
        ...
        let ways=((await r1.json()).elements||[]).filter(e=>e.type==='way');
        const mwCount=ways.filter(w=>w.tags&&(w.tags.highway==='motorway'||w.tags.highway==='trunk')).length;
        if(mwCount===0){const ru=await fetch(server,{method:'POST',body:'data='+encodeURIComponent(qUrban),signal:AbortSignal.timeout(25000)});if(!ru.ok)throw new Error(`HTTP ${ru.status}`);ways=((await ru.json()).elements||[]).filter(e=>e.type==='way');}
        const nodeIds=[...new Set(ways.flatMap(w=>w.nodes||[]))];
        const r2=await fetch(server,{method:'POST',body:'data='+encodeURIComponent(`[out:json][timeout:30];node(id:${nodeIds.join(',')});out skel;`),signal:AbortSignal.timeout(25000)});
        if(!r2.ok)throw new Error(`HTTP ${r2.status}`);
        return{elements:[...ways,...((await r2.json()).elements||[])]};
  ...
  static buildNetwork(osmData,lat,lon,W,H) {
    ...
    // Step 1: initial projection to find bounds
    const initScale=Math.min(W,H)/2/500;
    const cosLat=Math.cos(lat*Math.PI/180);
    const initProj=(nLat,nLon)=>({x:W/2+(nLon-lon)*111320*cosLat*initScale,y:H/2-(nLat-lat)*111320*initScale});
    const allPts=[];
    allRoadWays.forEach(w=>w.nodes.forEach(nid=>{const n=nodeMap[nid];if(n)allPts.push(initProj(n.lat,n.lon));}));
    // Step 2: compute auto-fit scale and centre
    let scale=initScale,cx=W/2,cy=H/2;
    if(allPts.length>1){
      const pad=50;
      ...
      const fitScale=Math.min((W-2*pad)/(maxX-minX),(H-2*pad)/(maxY-minY));
      scale=initScale*fitScale;
      cx=W/2-((minX+maxX)/2-W/2)*fitScale;
      cy=H/2-((minY+maxY)/2-H/2)*fitScale;
    }
    const project=(nLat,nLon)=>({x:cx+(nLon-lon)*111320*cosLat*scale,y:cy-(nLat-lat)*111320*scale});
    const nodePt=nid=>{const n=nodeMap[nid];return n?project(n.lat,n.lon):null;};
    if(mwWays.length>0||lkWays.length>0)
      return OSMFetcher._buildMotorway(mwWays,lkWays,nodePt,W,H);
  ...
  static _buildMotorway(mwWays,lkWays,nodePt,W,H) {
    // Pick dominant road
    const byRef={};
    mwWays.forEach(w=>{const ref=w.tags.ref||w.tags.name||'?';(byRef[ref]=byRef[ref]||[]).push(w);});
    const dominant=Object.values(byRef).sort((a,b)=>b.length-a.length)[0]||mwWays;

    // Trace two directional chains for the dual carriageway
    const byStart={}, endSet=new Set();
    dominant.forEach(w=>{byStart[w.nodes[0]]=w; endSet.add(w.nodes[w.nodes.length-1]);});
    const chainStarts=dominant.map(w=>w.nodes[0]).filter(n=>!endSet.has(n));
    const traceChain=s=>{
      const ways=[],seen=new Set(); let cur=byStart[s];
      while(cur&&!seen.has(cur.id)){seen.add(cur.id);ways.push(cur);cur=byStart[cur.nodes[cur.nodes.length-1]];}
      return ways;
    };
    const chainWays=chainStarts.slice(0,2).map(s=>traceChain(s));
    const chainPolylines=chainWays.map(wList=>{
      const pts=[]; wList.forEach(w=>{const wp=w.nodes.map(nid=>nodePt(nid)).filter(p=>p&&!isNaN(p.x));pts.push(...(pts.length?wp.slice(1):wp));});
      return pts;
    }).filter(p=>p.length>=2);

    const mainPts=chainPolylines[0]||[{x:0,y:H/2},{x:W,y:H/2}];
    const allMainPts=[...chainPolylines[0]||[],...chainPolylines[1]||[]];
    const nearMain=pt=>allMainPts.length?allMainPts.reduce((mn,p)=>Math.min(mn,Math.hypot(p.x-pt.x,p.y-pt.y)),Infinity):0;

    const mainRoads=[{
      id:0,name:'Dual Carriageway',
      x1:mainPts[0].x,y1:mainPts[0].y,x2:mainPts[mainPts.length-1].x,y2:mainPts[mainPts.length-1].y,
      angleRad:Math.atan2(mainPts[mainPts.length-1].y-mainPts[0].y,mainPts[mainPts.length-1].x-mainPts[0].x),
      lanes:3,lanesEachWay:3,speedLimit:120,roadType:'motorway',
      chainWays:chainWays,      // for rendering each way individually
      chain0:chainPolylines[0]||mainPts,
      chain1:chainPolylines[1]?[...chainPolylines[1]].reverse():chainPolylines[0]||mainPts,
    }];

    // Group connected link ways → each group = one physical slip road
    const nearbyLk=lkWays.filter(w=>{
      const pts=w.nodes.map(nid=>nodePt(nid)).filter(p=>p&&!isNaN(p.x));
      return pts.length>=2 && Math.min(nearMain(pts[0]),nearMain(pts[pts.length-1]))<=70;
    });
    ...
    const slipRoads=groups.map((grp,i)=>{
      ...
      const ds=nearMain(pts[0]), de=nearMain(pts[pts.length-1]);
      ...
      // renderPts: motorway-end → tip
      const renderPts=type==='off-ramp'?pts:[...pts].reverse();
      ...
      return{id:i,type,fromRoadId:0,toRoadId:0,hasMergeConflict:type==='on-ramp',
        ...
        curve:type==='off-ramp'?mid:[...mid].reverse(),
        renderPts};
    }).filter(Boolean);
