...
class Renderer {
  ...
  _motorway(){
    ...
    // 1. Slip verges (drawn under everything)
    net.slipRoads.forEach(s=>{poly(s.renderPts,SL+10,'#c8d8b8');});
    // 2. Motorway verge
    net.mainRoads.forEach(r=>{
      poly(r.chain0,MW+16,'#c8d8b8');
      poly(r.chain1,MW+16,'#c8d8b8');
    });
    // 3. Motorway surface
    net.mainRoads.forEach(r=>{
      poly(r.chain0,MW,'#5a6070');
      poly(r.chain1,MW,'#5a6070');
      poly(r.chain0,1.5,'rgba(255,255,255,0.85)');
      poly(r.chain1,1.5,'rgba(255,255,255,0.85)');
      poly(r.chain0,1,'rgba(255,255,255,0.3)',[9,7]);
      poly(r.chain1,1,'rgba(255,255,255,0.3)',[9,7]);
      // Green reservation stripe
      poly(r.chain0,4,'#7a9e6a');
      poly(r.chain1,4,'#7a9e6a');
      // Direction arrows
      [r.chain0,r.chain1].forEach(pts=>{
        if(!pts||pts.length<2)return;
        ctx.fillStyle='rgba(255,255,255,0.3)';let d=0;
        for(let i=1;i<pts.length;i++){
          const dx=pts[i].x-pts[i-1].x,dy=pts[i].y-pts[i-1].y;d+=Math.hypot(dx,dy);
          if(d>90){d=0;ctx.save();ctx.translate((pts[i].x+pts[i-1].x)/2,(pts[i].y+pts[i-1].y)/2);
            ctx.rotate(Math.atan2(dy,dx));ctx.beginPath();ctx.moveTo(6,0);ctx.lineTo(-3,-3);ctx.lineTo(-3,3);ctx.closePath();ctx.fill();ctx.restore();}
        }
      });
    });
    // 4. Slip roads on top
    net.slipRoads.forEach(s=>{
      poly(s.renderPts,SL,'#7a8190');
      poly(s.renderPts,1,'rgba(255,255,255,0.7)');
      const pts=s.renderPts; if(!pts||pts.length<2)return;
      const mi=Math.floor(pts.length*0.55);
      ...
      ctx.fillStyle=s.type==='on-ramp'?'#16a34a':'#d97706';
...
class OSMFetcher {
  ...
  static async geocode(query){
    const md=query.trim().match(/^(-?\d+\.?\d*)[,\s]+(-?\d+\.?\d*)$/);
    if(md)return{lat:+md[1],lon:+md[2],name:query};
  ...
  static async fetchRoads(lat,lon,radius=500){
    for(const b of OSMFetcher._BUNDLED){
      ...
      if(dist<1000){console.log(`Bundled: ${b.name}`);return b.data;}
    }
    const sv=['https://overpass-api.de/api/interpreter','https://overpass.kumi.systems/api/interpreter'];
    const dl=radius/111320,dn=radius/(111320*Math.cos(lat*Math.PI/180));
    const bb=`${lat-dl},${lon-dn},${lat+dl},${lon+dn}`;
    ...
    for(const s of sv){try{
      ...
      const mc=ws.filter(w=>w.tags&&(w.tags.highway==='motorway'||w.tags.highway==='trunk')).length;
      if(!mc){const ru=await fetch(s,{method:'POST',body:'data='+encodeURIComponent(qu),signal:AbortSignal.timeout(25000)});if(!ru.ok)throw new Error(`HTTP ${ru.status}`);ws=((await ru.json()).elements||[]).filter(e=>e.type==='way');}
  ...
  static buildNetwork(osmData,lat,lon,W,H) {
    ...
    // Fit scale to motorway ways only — slip roads stay within that bounds anyway
    ...
    const fitPts=[];
    mwWays.forEach(w=>w.nodes.forEach(nid=>{const n=nodeMap[nid];if(n)fitPts.push(initProj(n.lat,n.lon));}));
  ...
  static _buildMotorway(mwWays,lkWays,nodePt,W,H) {
    ...
    const traceChain=s=>{const ws=[],seen=new Set();let cur=byStart[s];while(cur&&!seen.has(cur.id)){seen.add(cur.id);ws.push(cur);cur=byStart[cur.nodes[cur.nodes.length-1]];}return ws;};
    const chainWays=chainStarts.slice(0,2).map(s=>traceChain(s));
    const chainPolylines=chainWays.map(wList=>{
      const pts=[];wList.forEach(w=>{const wp=w.nodes.map(nid=>nodePt(nid)).filter(p=>p&&!isNaN(p.x));pts.push(...(pts.length?wp.slice(1):wp));});return pts;
    }).filter(p=>p.length>=2);
    const mainPts=chainPolylines[0]||[{x:0,y:H/2},{x:W,y:H/2}];
    // ALL motorway nodes for proximity detection
    const allMWPts=[];
    ...
    // Snap to nearest point on chain polylines
    const snapToChains=pt=>{
      let best=pt,bestD=Infinity;
      chainPolylines.forEach(poly=>{
        for(let i=1;i<poly.length;i++){
          ...
          const sx=a.x+t*dx,sy=a.y+t*dy,d=Math.hypot(pt.x-sx,pt.y-sy);
    ...
    // Group link ways into connected assemblies = physical slip roads
    ...
    const slipRoads=groups.map((grp,i)=>{
      ...
      const branchRaw=type==='off-ramp'?pts[0]:pts[pts.length-1];
      const tPt=type==='off-ramp'?pts[pts.length-1]:pts[0];
      const bPt=snapToChains(branchRaw);
      const mid=type==='off-ramp'?pts.slice(1,-1):pts.slice(0,-1).slice(1);
      const renderPts=type==='off-ramp'?[bPt,...pts.slice(1,-1),tPt]:[tPt,...pts.slice(0,-1).reverse().slice(0,-1),bPt];
    ...
    console.log(`OSM: ${mwWays.length} mw ways, ${lkWays.length} lk ways → ${nearbyLk.length} nearby → ${groups.length} groups → ${slipRoads.length} slip roads`);
