...
class OSMFetcher {
  ...
  static async fetchRoads(lat,lon,radius=500) {
    // Check bundled data first — instant, complete, no API needed
    for(const b of OSMFetcher._BUNDLED){
      const dist=Math.hypot((lat-b.lat)*111320,(lon-b.lon)*111320*Math.cos(lat*Math.PI/180));
      if(dist<1000){
        console.log(`Using bundled data for ${b.name} (${dist.toFixed(0)}m away)`);
        return b.data;
      }
    }
    // Fall back to Overpass for other locations
